...
class Graph extends Component<IProps, {}> {
  ...
  componentDidUpdate() {
    if (this.table) {
      // Remove duplicate entries by filtering unique timestamps or by any other unique identifier if necessary.
      const uniqueData = Array.from(new Set(this.props.data.map(d => d.timestamp)))
        .map(timestamp => {
          return this.props.data.find(d => d.timestamp === timestamp);
        });
